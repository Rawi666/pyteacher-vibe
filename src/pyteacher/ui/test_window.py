"""
TestWindow for Strict Dictionary Test mode
"""
...
from .styles import (
    ...
    BOLD_LABEL_STYLE, FEEDBACK_LABEL_STYLE, STATS_LABEL_STYLE, WINDOW_STYLE
)
import os
...
class TestWindow(QWidget):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Strict Dictionary Test")
    ...
    def _init_ui(self):
        ...
        self.answered_label = QLabel("Answered: 0")
        self.answered_label.setStyleSheet(STATS_LABEL_STYLE)
        ...
        def load_file():
            ...
            try:
                ...
                self.controller = QuizTestController(self.questions)
    ...
    def _update_ui(self):
        ...
        if self.controller and self.controller.current_idx is not None:
            ...
            self.answered_label.setText(f"Answered: {stats['answered']}")
    ...
    def _show_results(self, stats):
        """Show results window when test is complete"""
        ...
        # For test mode, calculate accuracy based on correct vs total asked
        test_stats = stats.copy()
        test_stats['learned'] = stats['learned']  # This is the correct answers count
        perf = StatisticsCalculator.generate_performance_report(test_stats)
        results.set_results({**test_stats, 'accuracy': perf['accuracy']})
        results.exec()
    ...
    def _return_to_main_menu(self):
        ...
        self.close()
        window_manager.close_window('test')
